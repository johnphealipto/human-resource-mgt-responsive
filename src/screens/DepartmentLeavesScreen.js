...
const DepartmentLeavesScreen = ({ history, match }) => {
  const keyword = match.params.keyword || ''
	const pageNumber = match.params.pageNumber || 1
	const employees = 'mydepartmentapplications'
  ...
  const { userInfo } = userLogin
	...
	const departmentLeavesApps = useSelector(state => state.departmentLeavesApps)
  const { data, pages, page } = departmentLeavesApps
  ...
  useEffect(() => {
    ...
    if (userInfo  && (userInfo.role === 'Head Of Department')) {
       dispatch(getAllDeptLeaveApplications(pageNumber))
    } else {
      history.push('/')
    }
